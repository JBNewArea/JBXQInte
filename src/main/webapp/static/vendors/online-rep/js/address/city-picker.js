...
(function(factory) {
  ...
  function CityPicker(element, options) {
    ...
    this.$dropdown = null;
    this.options = $.extend({}, CityPicker.DEFAULTS, $.isPlainObject(options) &&
      options);
    this.active = false;
    this.dems = [];
    this.needBlur = false;
  ...
  CityPicker.prototype = {
    ...
    render: function() {
      var p = this.getPosition(),
        ...
        this.getWidthStyle(p.width + 18) + 'height:' +
        p.height + 'px;line-height:' + (p.height - 1) + 'px;">' +
        (placeholder ? '<span class="placeholder">' + placeholder +
          '</span>' : '') +
        '<span class="title"></span><div class="arrow"></div>' +
        '</span>',

        dropdown =
        '<div class="city-picker-dropdown" style="left:20px;top:49px;' +
    ...
    bind: function() {
      ...
      if (this.$province) {
        this.$province.on(EVENT_CHANGE, (this._changeProvince = $.proxy(
          function() {
            this.output(CITY);
    ...
    output: function(type) {
      ...
      code = (
        type === PROVINCE ? 86 :
        type === CITY ? this.$province && this.$province.find('.active')
        .data('code') :
        type === DISTRICT ? this.$city && this.$city.find('.active').data(
          'code') : code
      );
    ...
    simplize: function(address, type) {
      ...
      if (type === PROVINCE) {
        ...
        return address.length > 2 ? address.replace(/[市,区,县,旗]/g, '') :
          address;
      }
    },
    ...
    tab: function(type) {
      ...
      var $select = this['$' + type];
      var $tab = this.$dropdown.find('.city-select-tab > a[data-count="' +
        type + '"]');
      if ($select) {
        $selects.hide();
        $select.show();
  ...
  $.fn.citypicker = function(option) {
    ...
    return this.each(function() {
      ...
      if (!data) {
        ...
        options = $.extend({}, $this.data(), $.isPlainObject(option) &&
          option);
